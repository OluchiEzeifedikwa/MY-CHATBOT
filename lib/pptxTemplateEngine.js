...
const LABEL_MAP = {
  ...
  'NO OF REFERRING DOCTORS':      'doctorsReferred',
  'NO. OF REFERRING DOCTORS':     'doctorsReferred',
  'REFERRING DOCTORS':            'doctorsReferred',
  'TOTAL DOCTORS REFERRED':       'doctorsReferred',
  'DOCTORS':                      'doctorsReferred',
  ...
  'PATIENTS':                     'patients',
  'NO OF UNIQUE PATIENTS':        'patients',
  'NO. OF UNIQUE PATIENTS':       'patients',
...
function hasMonthText(text) {
  const lo = text.toLowerCase();
  return MONTH_NAMES.some(m => lo.includes(m)) || MONTH_ABBREVS.some(m => {
...
/** True when the row contains a month name/abbreviation in any non-label cell */
function rowHasMonth(cells) {
  return cells.slice(1).some(c => hasMonthText(extractText(c)));
}
...
function isComparisonTable(tableXml) {
  ...
  const monthCount = headerCells.slice(1).filter(c => hasMonthText(extractText(c))).length;
  return monthCount >= 2;
}
...
function valueForRow(label, templateCell, metrics) {
  // 1. Known KPI labels (doctors visited, patients, business, etc.) — fuzzy match
  const metric = resolveLabel(label);
...
/**
 * True when the table is a Test Wise Analysis table (has PROJ/TARGET + ACHIEVED/ACTUAL columns).
 */
function isTestWiseTable(tableXml) {
  ...
  const hasProj     = headerText.includes('PROJ') || headerText.includes('TARGET');
  const hasAchieved = headerText.includes('ACHIEVED') || headerText.includes('ACTUAL');
  return hasProj && hasAchieved;
}
...
/** Resolve a label string to a metrics key — exact match first, then fuzzy substring. */
function resolveLabel(label) {
  if (LABEL_MAP[label] !== undefined) return LABEL_MAP[label];
  // Fuzzy: label contains a known key, or a known key contains the label
...
function isKpiTable(tableXml) {
  ...
  return rows.some(r => {
    ...
    const label = extractText(cells[0]).toUpperCase().replace(/\s+/g, ' ').trim();
    return resolveLabel(label) !== undefined;
  });
}
...
/**
 * True when the table is a doctor ranking table (has DOCTORS/DOCTOR/REFERRING column + month columns).
 */
function isDoctorTable(tableXml) {
  ...
  const headerText = extractCells(rows[0][0])
    .map(c => extractText(c).toUpperCase()).join(' ');
  const hasDoctor = headerText.includes('DOCTOR') || headerText.includes('REFERRING');
  return hasDoctor && hasMonthText(headerText);
}
...
export async function detectTemplateMonth(buffer) {
  ...
  // Abbreviation → full month name index
  const abbrevToIdx = {
  ...
  for (const path of slides) {
    ...
    // Abbreviated month names (word-boundary to avoid false matches)
    for (const [abbr, idx] of Object.entries(abbrevToIdx)) {
      const n = (xml.match(new RegExp(`\\b${abbr}\\b`, 'g')) || []).length;
      if (n) {
        const full = MONTH_NAMES[idx];
        counts[full] = (counts[full] || 0) + n;
      }
    }
  }
